				...
				if (typeof entry[1] === "string") {
					...
					return (
						<div className="nav-item">
							<a className="nav-item-primary-link" to={link} key={navTitle}>{navTitle}</a>
						</div>
					);
				...
				// For a link with a submenu.
					...
					let dropdownContent: React.ReactNode = Object.entries(entry[1])
						...
						.map((entry2: [string, string]): React.ReactNode => {
							...
							return <a className="nav-item-dropdown-entry"
										 to={entryLink} key={entryTitle}>{entryTitle}</a>;
					...
					return (
						<div className="nav-item">
							<a className="nav-item-primary-link" to={primaryLink} key={navTitle}>{navTitle}</a>
